...
async def uloader(client, message):
    ...
    try:
        fsub = os.environ.get("UPDATE_CHANNEL")
    except:
        pass
    if fsub:
    ...
    is_downloading = True
    try:
        logchnl = int(os.environ.get("LOG_CHANNEL"))
    except:
        pass
    if logchnl:
    ...
    try:
        ...
        await c.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("Join Updates Channel 8", url="https://t.me/TGBotsProJect")
                    ]
                ]
            )
        )
        ...
        await c.send_message(
            chat_id=message.chat.id,
            text="Something went Wrong. Contact my [Creator](https://t.me/zautebot).",
            parse_mode="markdown",
            disable_web_page_preview=True)
        return False
